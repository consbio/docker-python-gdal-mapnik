...
RUN apt-get -y update && apt install -y wget build-essential libbz2-dev \
    ...
    liblzma-dev git python wget libharfbuzz-dev libfreetype6-dev libjpeg-dev \
    libproj-dev libpng-dev libwebp-dev libtiff5-dev libpq-dev \
    libcairo-dev

RUN wget https://dl.bintray.com/boostorg/release/1.66.0/source/boost_1_66_0.tar.gz
RUN tar -xf boost*
RUN (cd boost* && ./bootstrap.sh \
    --with-libraries=python,filesystem,system,regex,program_options && \
    ./b2 install)
